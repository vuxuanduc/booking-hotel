...
@section('content')
    ...
    <div class="height-100">
        ...
        <form id="form-add-room" action="{{ route('rooms.store') }}" method="post">
            @csrf
            <div class="row mb-4">
                <div class="col-6">
                    <div class="form-group mt-3">
                        ...
                        <select id="hotel_id" name="hotel_id" class="form-select mt-2">
                            ...
                            @foreach ($listHotels as $keyHotel => $valueHotel)
                                <option value="{{ $keyHotel }}">{{ $valueHotel }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group mt-3">
                        ...
                        <select id="room_type_id" name="room_type_id" class="form-select mt-2">
                            ...
                            @foreach ($listRoomTypes as $keyRoomType => $valueRoomType)
                                <option value="{{ $keyRoomType }}">{{ $valueRoomType }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group mt-3">
                        <label for="">Tên phòng</label> <span id="room_name_err" class="error"></span>
                        <input type="text" placeholder="Tên phòng..." id="room_name" name="room_name" class="form-control mt-2">
                    </div>
                </div>
                <div class="col-6">
                    <div class="form-group mt-3">
                        <label for="">Số lượng người</label> <span id="number_people_err" class="error"></span>
                        <input type="text" placeholder="Số lượng người..." name="number_people" id="number_people" class="form-control mt-2">
                    </div>
                    <div class="form-group mt-3">
                        <label for="">Giá phòng</label> <span id="price_err" class="error"></span>
                        <input type="text" placeholder="Giá phòng..." name="price" id="price" class="form-control mt-2">
                    </div>
                </div>
            </div>
            <div>
            ...
            <button id="btn-add-room" type="submit" class="btn text-white fw-bold mt-3" style="background-color: #86B817;">Thêm phòng</button>
        </form>
    </div>
@endsection
...
@section('script')
    ...
    <script>
        $(document).ready(function(){
            $('#form-add-room').on('submit' , function(event) {
