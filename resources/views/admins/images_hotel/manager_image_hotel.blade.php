...
@section('content')
    ...
    <div class="height-100">
                ...
                <tbody>
                    @foreach ($listImages as $keyImage => $valueImage)
                        <tr>
                            <td>{{ $valueImage->id }}</td>
                            <td>
                                <img src="{{ asset($valueImage->image_hotel) }}" width="120px" height="auto" alt="">
                            </td>
                            <td>
                                {{ $valueImage->image_hotel }}
                            </td>
                            <td>
                                <form action="{{ route('delete-image-hotel' , ['image_id' => $valueImage->id]) }}" method="post">
                                    @csrf
                                    @method("DELETE")
        ...
        <div class="mt-2 d-flex justify-content-start">  
            <a href="{{ route('create-image-hotel' , ['hotel_id' => $hotel_id]) }}" class="btn btn-primary">Thêm ảnh</a>
        </div>
    </div>
@endsection
